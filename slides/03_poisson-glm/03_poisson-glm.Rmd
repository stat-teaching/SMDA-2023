...
To understand **Overdispersion** we define the **scaled deviance** as:

\begin{align*}
D^{*} = \frac{D}{\phi} \\
D = -2[\log(\mathcal{L}_{current}) - (\log\mathcal{L}_{sat})]
\end{align*}

Where $D$ is the deviance that we already defined and $\phi$ is the dispersion parameter of the distribution. For Binomial and Poisson models $\phi = 1$ and $D = D^{*}$. We know that $D^{*} \sim \chi^2_{n - p - 1}$.

## Overdispersion

Without knowing $\phi$, we can estimate it using:
...
We can compute the ratio between `fit$deviance / fit$df.residual` = `r fit$deviance / fit$df.residual`:
...
The main problem of quasi-* models is that they are not a specific distribution family and there is not a likelihood function. For this reason, we cannot perform model comparison the standard AIC/BIC. See \@ref(Ver_Hoef2007-jc) for an overview.
...
We simulate a relationship `y ~ x` in the same way as the Poisson model but with a $\theta = 10$ 
